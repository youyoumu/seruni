...
export function setupServerWSForwarder<
  _CPush extends ClientPushEventMap,
  SPush extends ServerPushEventMap,
  CReq extends ClientReqEventMap,
  SRes extends ServerResEventMap,
...
type MyClientPushEventMap = {
  ping: CustomEvent;
};
type MyServerPushEventMap = {
  text_history: CustomEvent<{ text: string }>;
};

type MyClientReqEventMap = {
  req_config: CustomEvent<WithReqId>;
  req_config2: CustomEvent<WithReqId>;
};
type MyServerResEventMap = {
  res_config: CustomEvent<WithReqId<string>>;
  res_config2: CustomEvent<WithReqId<string>>;
};

type MyServerReqEventMap = {
  res_user_agent: CustomEvent<WithReqId<string>>;
};
type MyClientResEventMap = {
  res_user_agent: CustomEvent<WithReqId<string>>;
};
...
const { serverApi, clientApi } = createApiClient<
  MyClientPushEventMap,
  MyServerPushEventMap,
  MyClientReqEventMap,
  MyServerResEventMap,
  MyServerReqEventMap,
  MyClientResEventMap
...
clientApi.push("ping", new CustomEvent("ping"));
serverApi.addPushHandler("ping", (e) => {});

serverApi.push("text_history", new CustomEvent("text_history", { detail: { text: "" } }));
clientApi.addPushHandler("text_history", (e) => {
  e.detail.text;
});

clientApi.request("req_config");
serverApi.addReqHandler("req_config", (e) => {
  return "";
});
